...
--SCD Type 2: Enhancement Again

IF EXISTS (
    SELECT 1
    FROM sys.key_constraints
    WHERE name = 'UQ_DimCustomer_CustomerID'
    AND parent_object_id = OBJECT_ID('dw.DimCustomer')
)
BEGIN
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.key_constraints
    WHERE name = 'UQ_DimCustomer_Version'
      AND parent_object_id = OBJECT_ID('dw.DimCustomer')
)
BEGIN
    ALTER TABLE dw.DimCustomer
...
IF NOT EXISTS (
    SELECT 1
    FROM sys.indexes
    WHERE name = 'UX_DimCustomer_CurrentCustomerID'
      AND object_id = OBJECT_ID('dw.DimCustomer')
)
BEGIN
    CREATE UNIQUE INDEX UX_DimCustomer_CurrentCustomerID
        ON dw.DimCustomer(CustomerID)
        WHERE IsCurrent = 1;
END;
GO

--I have already done these, but gonna repeat for more precision (It's going to be better in that way)
--ik if statements are not too necessary, but it is a save for the future
...
IF EXISTS (
    ...
    WHERE name = 'CK_DimCustomer_Segment'
      AND parent_object_id = OBJECT_ID('dw.DimCustomer')
)
BEGIN
    ALTER TABLE dw.DimCustomer
    DROP CONSTRAINT CK_DimCustomer_Segment;
END;
GO

ALTER TABLE dw.DimCustomer
ADD CONSTRAINT CK_DimCustomer_Segment
CHECK (
    CustomerSegment IN (
        'Unknown',
        'Standard',
...
--Classic Special Case For The Null One!
...
SELECT
    ...
    cb.RegistrationDate AS EffectiveStartDate,

    CAST('99991231' AS DATE) AS EffectiveEndDate,
